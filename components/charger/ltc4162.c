...
esp_err_t ltc4162_write_register(uint8_t reg, uint8_t *data, size_t len) {
    ...
    return _mcu_i2c_write(LTC4162_I2C_ADDRESS, reg, data, len) ? ESP_OK : ESP_FAIL;
}
...
esp_err_t ltc4162_debug_monitor(void)
{
    ...
    if (sys & (1 << 8)) ESP_LOGW(TAG, "Charging active");
    if (sys & (1 << 7)) ESP_LOGE(TAG, "Cell count error");
    if (sys & (1 << 5)) ESP_LOGW(TAG, "No RT resistor");
    if (sys & (1 << 4)) ESP_LOGE(TAG, "Thermal shutdown!");
    if (sys & (1 << 3)) ESP_LOGE(TAG, "VIN overvoltage!");
    if (sys & (1 << 2)) ESP_LOGI(TAG, "VIN > VBAT");
    if (sys & (1 << 1)) ESP_LOGI(TAG, "VIN > 4.2V");
    if (sys & (1 << 0)) ESP_LOGI(TAG, "INTVCC OK");
