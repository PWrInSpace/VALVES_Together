...
esp_err_t ltc4162_read_register(uint8_t reg, uint8_t *data, uint8_t len) {
    if (LTC_config.i2c_read == NULL) return ESP_ERR_INVALID_STATE;
    if (len == 0 || data == NULL) return ESP_ERR_INVALID_ARG;
    
    return LTC_config.i2c_read(LTC_config.i2c_address, reg, data, len) ? ESP_OK : ESP_FAIL;
}

esp_err_t ltc4162_write_register(uint8_t reg, uint8_t *data, uint8_t len) {
    if (LTC_config.i2c_write == NULL) return ESP_ERR_INVALID_STATE;
    if (len == 0 || data == NULL) return ESP_ERR_INVALID_ARG;

    return LTC_config.i2c_write(LTC_config.i2c_address, reg, data, len) ? ESP_OK : ESP_FAIL;
}
...
esp_err_t ltc4162_init(const ltc4162_config_t *config) {
    ...
    if (write_reg16(LTC4162_REG_CONFIG_BITS, telemetry_cmd) != ESP_OK) {
        ESP_LOGE(TAG, "Hardware communication error: Failed to write config register 0x%02X", LTC4162_REG_CONFIG_BITS);
        return ESP_FAIL;
    }

    int16_t config_check = 0;
    if (read_reg16_checked(LTC4162_REG_CONFIG_BITS, &config_check) != ESP_OK) {
        ESP_LOGE(TAG, "Hardware communication error: Failed to read back config register 0x%02X", LTC4162_REG_CONFIG_BITS);
        return ESP_FAIL;
    }
    if ((config_check & (1 << 2)) == 0) {
        ESP_LOGE(TAG, "Verification failed: force_telemetry_on bit did not clear or set correctly");
        return ESP_FAIL;
    }
    ...
    if (LTC_config.int_pin != GPIO_NUM_NC && LTC_config.int_pin >= 0) {
        ...
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "INT pin configuration failed (code: %s), continuing without hardware interrupts", esp_err_to_name(err));
        }
    }

    LTC_initialized = true;
    ESP_LOGI(TAG, "LTC4162 initialized successfully at address 0x%02X", LTC_config.i2c_address);
    return ESP_OK;
}

esp_err_t ltc4162_deinit(void) {
  if (!LTC_initialized) return ESP_ERR_INVALID_STATE;

  if (ltc4162_enable_interrupts(LTC4162_ALERT_LIMIT_NONE, LTC4162_ALERT_STATE_NONE, LTC4162_ALERT_STATUS_NONE) != ESP_OK) {
    ESP_LOGW(TAG, "Could not clear hardware alert masks (I2C error), forcing software deinit");
  }
...
esp_err_t ltc4162_enable_interrupts(uint16_t limit_mask, uint16_t state_mask, uint16_t status_mask) {
    ...
    if (write_reg16(LTC4162_REG_EN_LIMIT_ALERTS, limit_mask) != ESP_OK) return ESP_FAIL;
    if (write_reg16(LTC4162_REG_EN_CHARGER_STATE_ALERTS, state_mask) != ESP_OK) return ESP_FAIL;
    if (write_reg16(LTC4162_REG_EN_CHARGE_STATUS_ALERTS, status_mask) != ESP_OK) return ESP_FAIL;
...
esp_err_t ltc4162_get_alerts(uint16_t *limit_alerts, uint16_t *state_alerts, uint16_t *status_alerts) {
    if (!LTC_initialized) return ESP_ERR_INVALID_STATE;
    if (!limit_alerts || !state_alerts || !status_alerts) return ESP_ERR_INVALID_ARG;

    int16_t raw_limit = 0, raw_state = 0, raw_status = 0;

    if (read_reg16_checked(LTC4162_REG_LIMIT_ALERTS, &raw_limit) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(LTC4162_REG_CHARGER_STATE_ALERTS, &raw_state) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(LTC4162_REG_CHARGE_STATUS_ALERTS, &raw_status) != ESP_OK) return ESP_FAIL;
...
esp_err_t ltc4162_set_suspend(bool suspend) {
    ...
    if (read_reg16_checked(LTC4162_REG_CONFIG_BITS, &current_config) != ESP_OK) return ESP_FAIL;

    if (suspend) current_config |= (1 << 5);
    else current_config &= ~(1 << 5);

    if (write_reg16(LTC4162_REG_CONFIG_BITS, (uint16_t)current_config) != ESP_OK) {
        return ESP_FAIL;
    }

    ESP_LOGI(TAG, "Charger status updated: %s", suspend ? "SUSPENDED" : "RESUMED");
    return ESP_OK;
}
  
esp_err_t read_charger_data(ltc4162_charger_data_t *charger_data) {
    if (charger_data == NULL) return ESP_ERR_INVALID_ARG;
    if (!LTC_initialized) {
    ...
    if (read_reg16_checked(0x3A, &vbat_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x3B, &vin_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x3C, &vout_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x3D, &iin_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x3E, &ibat_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x3F, &die_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x39, &sys_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x34, &charger_state_raw) != ESP_OK) return ESP_FAIL;
    if (read_reg16_checked(0x43, &chem_cells) != ESP_OK) return ESP_FAIL;
    ...
    charger_data->vbat = vbat_raw * (cell_count * 192.4e-6f); // Battery voltage (V)
    ...
    charger_data->die_temp = die_raw * 0.0215f - 264.4f; // IC die temperature (°C)
    charger_data->charger_status = (sys_raw & (1 << 8)) != 0; // If charger is actively power pathing/running
    charger_data->charger_state = (charger_state_raw == 2) ? 1 : 0; // 1 for active charging, 0 otherwise
