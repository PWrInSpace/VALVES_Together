...
bool save_text(const char* path, BoardData_t* data) {
    ...
    for (uint32_t i = 0; i < BUFFER_SAMPLES; i++) {
        fprintf(f, "%u,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%.2f,%d,%d,%d,%d,%d,%d,%.2f,%.2f,%.2f,%.2f,%d,%d\n",
                power_time(),
                ...
                valve1_state,
                valve2_state,
                data[i].chargerData.vbat,
                data[i].chargerData.vin,
                data[i].chargerData.ibat,
                data[i].chargerData.iin,
                data[i].chargerData.die_temp,
                data[i].chargerData.vout,
...
bool add_header(const char* path) {
    ...
    fprintf(f, "PowerTime,Temp1,Temp2,Temp3,Press1,Press2,Press3,Termistor,IgnCon,IJumpFlag,Valve1State,Valve2State,Vbat,Vin,Ibat,Iin,DieTemp,Vout,ChargerStatus,ChargerState\n");
