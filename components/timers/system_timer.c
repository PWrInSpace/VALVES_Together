...
bool sys_timer_start(sys_timer_id_t id, uint32_t miliseconds,
  ...
  if (type == TIMER_TYPE_ONE_SHOT) {
    esp_timer_start_once(gb.timers[index].timer_handle,
                         MS_TO_MICRO(miliseconds));
  } else {
    esp_timer_start_periodic(gb.timers[index].timer_handle,
                             MS_TO_MICRO(miliseconds));
  }
...
bool sys_timer_restart(sys_timer_id_t id, uint64_t timeout) {
  ...
  if (esp_timer_restart(gb.timers[index].timer_handle, MS_TO_MICRO(timeout)) !=
      ESP_OK) {
...
bool sys_timer_get_expiry_time(sys_timer_id_t id, uint64_t *expiry) {
  ...
  if (esp_timer_get_expiry_time(gb.timers[index].timer_handle, expiry) !=
      ESP_OK) {
    return false;
  }
